...
class DeduplicateWatchedAnimeTests(TestCase):
    """Merging duplicate active-tracking anime rows (highest progress wins)."""

    def setUp(self):
        """Track one show under one user."""
        self.user = get_user_model().objects.create_user(
            username="anime-dedup-user",
            password="password",
        )
    ...
    def test_two_active_rows_are_merged_keeping_the_highest_progress(self):
        """A stray lower-progress duplicate is dropped."""
        with patch("integrations.tasks.sync_mal_status.delay"):
            low = Anime.objects.create(
                item=self.item,
                user=self.user,
                status=Status.IN_PROGRESS.value,
                progress=3,
            )
            high = Anime.objects.create(
                item=self.item,
                user=self.user,
                status=Status.IN_PROGRESS.value,
                progress=9,
        ...
        groups = list(duplicate_active_anime_groups(user=self.user))
        self.assertEqual(len(groups), 1)

        result = deduplicate_watched_anime_task(self.user.id)

        self.assertEqual(result, {"merged": 1, "skipped": 0, "unresolved": []})
        self.assertFalse(Anime.objects.filter(pk=low.pk).exists())
        self.assertTrue(Anime.objects.filter(pk=high.pk).exists())

    def test_multiple_completed_rewatches_are_never_touched(self):
        """Two completed rows for the same show are legitimate rewatch history."""
        with patch("integrations.tasks.sync_mal_status.delay"):
            first = Anime.objects.create(
                item=self.item,
                user=self.user,
                status=Status.COMPLETED.value,
                progress=13,
            )
            second = Anime.objects.create(
                item=self.item,
                user=self.user,
                status=Status.COMPLETED.value,
                progress=13,
        ...
        self.assertEqual(list(duplicate_active_anime_groups(user=self.user)), [])

        result = deduplicate_watched_anime_task(self.user.id)

        self.assertEqual(result, {"merged": 0, "skipped": 0, "unresolved": []})
        self.assertTrue(Anime.objects.filter(pk=first.pk).exists())
        self.assertTrue(Anime.objects.filter(pk=second.pk).exists())

    def test_a_completed_row_with_a_stale_lower_active_row_is_merged(self):
        """A 12/12 completed row plus a stray 11/12 row is a duplicate, not a rewatch."""
        with patch("integrations.tasks.sync_mal_status.delay"):
            stale = Anime.objects.create(
                item=self.item,
                user=self.user,
                status=Status.IN_PROGRESS.value,
                progress=11,
        ...
        self.assertEqual(result, {"merged": 1, "skipped": 0, "unresolved": []})
        self.assertFalse(Anime.objects.filter(pk=stale.pk).exists())
        self.assertTrue(Anime.objects.filter(pk=completed.pk).exists())

    def test_unknown_user_id_is_a_no_op(self):
        """A missing user returns a zeroed result instead of raising."""
        result = deduplicate_watched_anime_task(999999)

        self.assertEqual(result, {"merged": 0, "skipped": 0, "unresolved": []})


class DeduplicateWatchedAnimeViewTests(TestCase):
    """The settings-page button queues the dedup task, nothing sooner."""

    def setUp(self):
        """Log in as a plain user."""
        self.user = get_user_model().objects.create_user(
            username="anime-dedup-view-user",
            password="password",
        )
        self.client.force_login(self.user)

    def test_endpoint_queues_the_task(self):
        from django.urls import reverse

        with patch(
            "app.tasks_anime_library_repair.deduplicate_watched_anime_task.delay",
        ) as mock_delay:
            response = self.client.post(reverse("deduplicate_watched_anime"))
